...
def show(user_id, user_role_ids=None):
    ...
    if has_management_role:
        ...
        with tab3:
            ...
            # Patient and Coordinator Monthly Summaries are now shown by show_coordinator_tasks_tab()
            # above to avoid duplication. The Coordinator Tasks Table is also included there.
            # Note: For advanced editing with save functionality, use the Admin Dashboard.
...
def show_daily_task_log(user_id, role="coordinator"):
    ...
    try:
        ...
        if "coordinator_task_id" in display_df.columns:
            display_df["_task_id"] = display_df["coordinator_task_id"]
        elif "provider_task_id" in display_df.columns:
