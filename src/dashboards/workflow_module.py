...
from src.database import (
    ...
    ensure_monthly_coordinator_tasks_table,
    get_db_connection,
...
def create_workflow_instance(template_id, patient_id, coordinator_id, notes=None):
    ...
    try:
        tmpl = conn.execute(
            "SELECT template_name FROM workflow_templates WHERE template_id = ?",
            (template_id,),
        ).fetchone()
        template_name = (
            tmpl["template_name"]
            if tmpl is not None and "template_name" in tmpl.keys()
            else None
        )
        ...
        try:
            ...
            if coord_row:
                coord_name = (
                    f"{coord_row['first_name']} {coord_row['last_name']}".strip()
                )
        except Exception:
            coord_name = None
...
def save_progress_step(
    instance_id, step_id, coordinator_id, duration_minutes=0, notes=""
):
...
def complete_workflow_step(
    ...
    try:
        ...
        if not workflow_info:
            workflow_info = conn.execute(
                "SELECT ct.patient_id FROM coordinator_tasks_2025_09 ct WHERE ct.task_type LIKE 'WORKFLOW_START|' || ? LIMIT 1",
                (instance_id,),
            ).fetchone()
        if not workflow_info:
...
def get_workflow_progress(instance_id):
    ...
    try:
        ...
        if (
            wi_row is not None
            and "created_at" in wi_row.keys()
            and wi_row["created_at"]
        ):
            try:
        ...
        for s in steps:
            ...
            if table_exists:
                try:
                    ct_row = conn.execute(
                        f"SELECT coordinator_task_id, task_date, duration_minutes, notes FROM {table_name} WHERE task_type = ? LIMIT 1",
                        (f"WORKFLOW_STEP|{instance_id}|{step_id}",),
                    ).fetchone()
                except Exception:
                    ct_row = None
            ...
            try:
                ...
                if (
                    wi_row is not None
                    and step_col in wi_row.keys()
                    and wi_row[step_col]
                ):
                    completed = True
                    completion_notes = (
                        wi_row[notes_col] if notes_col in wi_row.keys() else None
                    )
            ...
            result.append(
                {
                    "instance_id": wi_row["instance_id"]
                    if wi_row is not None and "instance_id" in wi_row.keys()
                    else instance_id,
...
def render_workflow_table(
    df, display_cols, empty_message="No ongoing workflows found."
):
    if not df.empty:
        st.dataframe(
            df[display_cols],
            ...
            column_config={
                "instance_id": st.column_config.TextColumn(
                    "Workflow Instance ID", width="small"
                ),
                "patient_name": st.column_config.TextColumn("Patient", width="medium"),
                "workflow_type": st.column_config.TextColumn(
                    "Workflow Type", width="large"
                ),
                "coordinator_id": st.column_config.TextColumn(
                    "Coordinator ID", width="small"
                ),
                "coordinator_name": st.column_config.TextColumn(
                    "Coordinator Name", width="medium"
                ),
                "current_owner_user_id": st.column_config.TextColumn(
                    "Current Owner User ID", width="small"
                ),
                "current_owner_name": st.column_config.TextColumn(
                    "Current Owner Name", width="medium"
                ),
                "current_step": st.column_config.NumberColumn(
                    "Current Step", width="small"
                ),
                "total_steps": st.column_config.NumberColumn(
                    "Total Steps", width="small"
                ),
                "step_progress": st.column_config.TextColumn(
                    "Step Progress", width="medium"
                ),
    ...
    with top_col1:
        ...
        with st.form("quick_start_workflow_form"):
            ...
            try:
                conn = get_db_connection()
                cursor = conn.execute(
                    "SELECT template_name FROM workflow_templates ORDER BY template_name"
                )
            ...
            submitted = st.form_submit_button(
                "Start Workflow", use_container_width=True
            )
            ...
            if submitted:
                if selected_patient != "Select Patient..." and task_notes.strip():
                    try:
                        ...
                        if success:
                            st.success(
                                f"Workflow started: {selected_workflow} for {selected_patient}"
                            )
                            st.rerun()
                        else:
        ...
        if workflow_options:
            ...
            if selected_instance_id:
                ...
                try:
                    ...
                    if current_user:
                        ...
                        user_name = (
                            ...
                            if "full_name" in current_user.keys()
                            else current_user.get("full_name", "Unknown User")
                        )
                        # Check if user is Jan or Jose (supervisors/managers)
                        can_assign_workflows = user_name in [
                            "Estomo, Jan",
                            "Soberanis, Jose",
                        ]
                except Exception:
                ...
                with btn_col2:
                    if st.button(
                        "Complete Next Step",
                        key=f"complete_next_{selected_instance_id}",
                    ):
                        try:
                ...
                if can_assign_workflows:
                    ...
                    try:
                        ...
                        all_coordinators = get_users_by_role(
                            36
                        ...
                        if current_user:
                            current_user_name = (
                                current_user["full_name"]
                                if "full_name" in current_user.keys()
                                else current_user.get("full_name", "")
                            )
                        ...
                        if coordinator_options:
                            ...
                            with assignment_col2:
                                if st.button(
                                    "Assign",
                                    key=f"assign_workflow_{selected_instance_id}",
                                ):
                                    try:
                                        target_coordinator_id = coordinator_options[
                                            selected_coord_name
                                        ]
                                        msg = assign_workflow_to_user(
                                            selected_instance_id, target_coordinator_id
                                        )
                                        st.success(msg)
                                        st.rerun()
                                    except Exception as e:
                ...
                with st.form(
                    f"add_note_form_{selected_instance_id}", clear_on_submit=True
                ):
                    note = st.text_area(
                        "Add Note",
                        key=f"note_{selected_instance_id}",
                        height=68,
                    ...
                    if submitted and note.strip():
                        try:
                            msg = add_note_to_workflow(
                                selected_instance_id, user_id, note
                            )
                            st.success(msg)
                            st.rerun()
                        except Exception as e:
                ...
                st.markdown("<div style='height: 0.5em'></div>", unsafe_allow_html=True)
                if st.button(
                    "Complete Workflow", key=f"complete_workflow_{selected_instance_id}"
                ):
                    try:
...
def render_workflow_steps(instance_id, user_id, user_role_ids):
    ...
    if not progress:
        ...
        return
    st.subheader(
        f"Resume Workflow - {progress[0]['template_name'] if progress and 'template_name' in progress[0] else instance_id}"
    )
    ...
    for p in progress:
        ...
        if completed:
            ...
            timer_display_col, start_stop_col, reset_col, duration_col = st.columns(
                [1, 1, 1, 1]
            )
            ...
            if existing_notes:
                ...
                st.text_area(
                    ...
                    height=100,
                    disabled=True,
                    key=f"existing_{instance_id}_{step_id}",
                    label_visibility="collapsed",
                )
            ...
            st.markdown(
                f"**Add New Notes** (will be saved as: ▶ [{today}] your_notes_here)"
            )
            ...
            with save_col:
                if st.button(
                    "Save Progress",
                    key=f"save_progress_{instance_id}_{step_id}",
                    use_container_width=True,
                    type="secondary",
                ):
                    try:
                        ...
                        duration = st.session_state.get(
                            f"duration_{instance_id}_{step_id}", 30
                        )
                        if notes.strip():  # Only save if there are notes
                            success = save_progress_step(
                                instance_id,
                                step_id,
                                user_id,
                            ...
                            if success:
                                ...
                                st.session_state[
                                    f"clear_notes_{instance_id}_{step_id}"
                                ] = True
                                st.rerun()
                            else:
            ...
            with complete_col:
                if st.button(
                    f"Complete Step",
                    key=f"complete_step_{instance_id}_{step_id}",
                    use_container_width=True,
                    type="primary",
                ):
                    try:
...
def assign_workflow_to_me(instance_id, user_id):
    ...
    try:
        conn.execute(
            "UPDATE workflow_instances SET coordinator_id = ? WHERE instance_id = ?",
            (user_id, instance_id),
        )
        conn.commit()
...
def assign_workflow_to_user(instance_id, target_coordinator_id):
    ...
    try:
        ...
        coord_row = conn.execute(
            "SELECT full_name FROM users WHERE user_id = ?", (target_coordinator_id,)
        ).fetchone()
        coord_name = (
            coord_row["full_name"]
            if coord_row and "full_name" in coord_row.keys()
            else coord_row.get("full_name", "Unknown")
            if coord_row
            else "Unknown User"
        )
        ...
        conn.execute(
            "UPDATE workflow_instances SET coordinator_id = ? WHERE instance_id = ?",
            (target_coordinator_id, instance_id),
        )
        conn.commit()
...
def add_note_to_workflow(instance_id, user_id, note):
    ...
    try:
        ...
        """,
            (user_id, patient_id, f"WORKFLOW_NOTE|{instance_id}", note),
        )
        conn.commit()
...
from src.utils.workflow_utils import create_workflow_task, get_ongoing_workflows
