...
# Set environment variables for sandbox OAuth
$env:ENVIRONMENT = "sandbox"
$env:STREAMLIT_SERVER_PORT = "8502"

# Copy sandbox secrets if they exist
if (Test-Path ".streamlit\secrets.sandbox.toml") {
    Copy-Item ".streamlit\secrets.sandbox.toml" ".streamlit\secrets.toml" -Force
    Write-Host "Using sandbox OAuth configuration (Port 8502)" -ForegroundColor Yellow
}

Write-Host "OAuth Redirect URI: http://localhost:8502/oauth/callback" -ForegroundColor Cyan

# Run the Streamlit app on sandbox port
try {
