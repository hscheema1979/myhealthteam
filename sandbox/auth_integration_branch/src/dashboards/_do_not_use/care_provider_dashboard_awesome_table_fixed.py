...
try:
    ...
    print("Warning: streamlit_awesome_table not available, using fallback")
...
def show(user_id):
    ...
    # Using optimized query with summary tables
    try:
    ...
    # Using optimized query with provider_region_summary table
    all_regions = []
    all_zip_codes = []
    
    try:
        # First, try to get regions using the optimized summary table
        conn = database.get_db_connection()
        cursor = conn.execute("""
            SELECT DISTINCT 
                prs.region_id, 
            ...
            ORDER BY prs.region_name, prs.city, prs.zip_code
        """, (provider_id,))
        regions = cursor.fetchall()
        conn.close()
        
        # If no regions found in summary table, fall back to direct query from regions table
        if not regions:
            try:
                conn = database.get_db_connection()
                cursor = conn.execute("""
                    ...
                    GROUP BY r.region_id, r.region_name, r.zip_code, r.city, r.state
                    ORDER BY r.region_name, r.city, r.zip_code
                """, (provider_id,))
                regions = cursor.fetchall()
                conn.close()
            except Exception as e2:
                print(f"Error in fallback region query: {e2}")
        
        # Format regions as (region_id, display_text) with patient count
        all_regions = [(r[0], f"{r[1]} - {r[3]}, {r[4]} {r[2]} [{r[6]}]") for r in regions]  # (region_id, formatted_name with count)
        
        # Get unique zip codes using the optimized summary table
        conn = database.get_db_connection()
        cursor = conn.execute("""
            SELECT DISTINCT 
                pzs.zip_code, 
        ...
        # If no zip codes found in summary table, fall back to direct query
        if not zip_codes:
            try:
                conn = database.get_db_connection()
                cursor = conn.execute("""
                ...
                print(f"Error in fallback zip code query: {e2}")
        
        # Format zip codes as (zip_code, display_text) with patient count
        all_zip_codes = [(z[0], f"{z[0]} - {z[1]}, {z[2]} [{z[3]}]") for z in zip_codes]  # (zip_code, formatted_name with count)
        
    except Exception as e:
        print(f"Error getting provider regions/zip codes: {e}")
        # Provide fallback data to show that the system is working
        all_regions = [(1, "San Francisco Bay Area - San Francisco, CA 94102 [0]")]
        all_zip_codes = [("94102", "94102 - San Francisco, CA [0]")]
    ...
    if selected_region != 'All Regions':
        # Extract region_id from the selected region display text
        # Format is: "Region Name - City, State ZipCode [Patient Count]"
        try:
            # Extract the region_id from the all_regions list by matching the display text
            selected_region_id = None
            for region_id, display_text in all_regions:
                if display_text == selected_region:
                    selected_region_id = region_id
                    break
            
            if selected_region_id:
                # Filter patients by region using the patient_region_mapping table
                try:
                    conn = database.get_db_connection()
                    cursor = conn.execute("""
                        ...
                        WHERE p.region_id = ?
                    """, (selected_region_id,))
                    patient_ids_in_region = [row[0] for row in cursor.fetchall()]
                    conn.close()
                    
                    # Filter patients to only those in this region
                    filtered_patients = [p for p in filtered_patients if p.get('patient_id') in patient_ids_in_region]
                except Exception as e:
                    print(f"Error filtering by region: {e}")
        except Exception as e:
            print(f"Error processing region filter: {e}")
