...
def save_daily_task(provider_id, patient_id, task_date, task_description, notes, billing_code=None):
    ...
    try:
        ...
        conn.execute("""
            ...
            (task_id, provider_id, patient_id, task_date, notes, minutes_of_service, task_description, billing_code_description, source_system, imported_at)
...
def save_coordinator_task(coordinator_id, patient_id, task_date, task_description, duration_minutes, notes):
    ...
    try:
        ...
        conn.execute("""
            ...
            (task_id, patient_id, coordinator_id, task_date, duration_minutes, task_type, notes, source_system, imported_at)
            VALUES (NULL, ?, ?, ?, ?, ?, ?, 'DATA_ENTRY', CURRENT_TIMESTAMP)
...
def transfer_onboarding_to_patient_table(onboarding_id):
    ...
    try:
        ...
        if existing_patient:
            ...
            params.append(onboarding_dict['patient_id'])
...
def get_user_by_email(email):
    """Get user by email address for OAuth authentication"""
    conn = get_db_connection()
    try:
...
def add_oauth_user(user_data):
    """
    Add a new user from OAuth authentication
    ...
    try:
        ...
        cursor = conn.execute("""
            INSERT INTO users (
                ...
                status, hire_date, oauth_provider, google_id, picture_url
        ...
        """, (
            ...
            user_data.get('picture_url', '')
        ))
        
        user_id = cursor.lastrowid
        
        # Assign default role (you may want to customize this)
        # For now, assign a basic user role - adjust role_id as needed
        default_role_id = 39  # Data Entry role as default
        conn.execute("INSERT INTO user_roles (user_id, role_id) VALUES (?, ?)", (user_id, default_role_id))
        
        conn.commit()
        return user_id
        
    except sqlite3.Error as e:
        print(f"Database error in add_oauth_user: {e}")
        conn.rollback()
        return None
    finally:
        conn.close()
...
def update_user_oauth_info(user_id, oauth_data):
    """Update user's OAuth information"""
    conn = get_db_connection()
    try:
        conn.execute("""
            UPDATE users 
            SET google_id = ?, picture_url = ?, oauth_provider = ?
            WHERE user_id = ?
        """, (
            oauth_data.get('google_id', ''),
            oauth_data.get('picture_url', ''),
            oauth_data.get('oauth_provider', 'google'),
            user_id
        ))
        conn.commit()
    except sqlite3.Error as e:
...
def authenticate_user(email, password=None):
    """Authenticate user with email and password (password optional for testing)"""
    conn = get_db_connection()
    try:
        # For testing purposes, if no password is provided, authenticate by email only
        if password is None:
            user = conn.execute("""
...
def update_all_users_password(default_password="pass123"):
...
def get_user_by_email_simple(email):
    """Get user by email for simple lookup"""
    conn = get_db_connection()
    try:
        user = conn.execute("""
            SELECT user_id, username, email, first_name, last_name, full_name, status 
