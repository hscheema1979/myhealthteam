...
def show(user_id):
    st.title("Care Provider Dashboard - Enhanced Zip Code Filtering")
    ...
    if filtered_patients:
        try:
            ...
            # Display with AG Grid for better filtering capabilities
            st.subheader("Patient Data Table")
            
            # Configure AG Grid
            gb = GridOptionsBuilder.from_dataframe(display_df)
            gb.configure_default_column(editable=False, filterable=True, sortable=True)
            gb.configure_column("First Name", headerName="First Name", width=150)
            gb.configure_column("Last Name", headerName="Last Name", width=150)
            gb.configure_column("Address", headerName="Address", width=250)
            gb.configure_column("Contact Info", headerName="Contact Info", width=200)
            gb.configure_column("Status", headerName="Status", width=120)
            
            gb.configure_grid_options(domLayout='normal')
            gb.configure_pagination(paginationPageSize=10, paginationPageSizeSelector=[10, 25, 50, 100])
