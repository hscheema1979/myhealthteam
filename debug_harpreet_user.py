...
def debug_harpreet_account():
    ...
    try:
        ...
        harpreet_user = conn.execute(
            "SELECT user_id, full_name, email, username FROM users WHERE email = ? OR username LIKE ?",
            ("harpreet@myhealthteam.org", "%harpreet%")
        ).fetchall()
        ...
        if harpreet_user:
            ...
            # Test with admin role only (34)
            if 34 in role_ids:
                print(f"\nTesting with Admin role only [34]:")
                admin_df = get_workflows_for_reassignment(user_id, [34])
                print(f"Admin workflows: {len(admin_df)}")
            ...
            print("❌ No user found matching harpreet@myhealthteam.org")
            
            # Show all admin users
            print("\nAll admin users (role_id = 34):")
