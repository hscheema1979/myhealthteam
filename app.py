...
st.set_page_config(page_title="Zen Medicine", layout="wide", initial_sidebar_state="expanded")
import sqlite3
import os
...
def render_help_panel():
    guide_path = os.path.join("scripts", "automation", "outputs", "reports", "dashboard_user_guide.html")
    try:
        ...
        html_content = re.sub(r'src\s*=\s*"(file:[^"]+)"', replace_file_uri_with_data_uri, html_content)
...
def render_screenshot_gallery():
    ...
    try:
        ...
        for fname in files:
            ...
            caption = fname.replace("Screenshot Capture - ", "").replace(".png", "").replace(".jpg", "")
...
def render_role_screenshot_gallery(role: str):
    ...
    for d in candidates:
        if os.path.isdir(d):
            imgs = sorted([f for f in os.listdir(d) if f.lower().endswith((".png", ".jpg", ".jpeg"))])
            if imgs:
                chosen = (d, imgs)
                break
    if not chosen:
        st.info(f"No screenshots found for role='{role}'. Checked: {', '.join(candidates)}")
        return
...
def main():
    ...
    if st.session_state.get("show_help"):
        ...
        render_help_panel()
        st.button("Close Help", key="close_help", on_click=lambda: st.session_state.update({"show_help": False}))
        st.markdown("---")
...
def render_help_markdown(role: str, section: str):
    ...
    if search_query:
        ...
        if hits:
            ...
            for s in hits:
                st.markdown(f"- [{s.replace('_',' ').title()}](/?page=help&role={role}&section={s})")
        else:
    ...
    if section in ('index', 'toc'):
        ...
        if os.path.isdir(role_dir):
            ...
            if files:
                ...
                st.markdown("No sections found yet. Please add markdown files under " + role_dir)
        return
    ...
    if os.path.isfile(md_path):
        ...
        st.info(f"Help not yet authored for role='{role}', section='{section}'. Expected at {md_path}.")
        # List available sections for this role
        if os.path.isdir(role_dir):
            files = [f.replace('.md','') for f in os.listdir(role_dir) if f.lower().endswith('.md')]
            if files:
...
def render_provider_help_elements():
    ...
    st.markdown("Use these tabs to work your panel: My Patients, Phone Reviews, Task Review. The help below explains each section and the color meanings.")

    st.subheader("Color legend (status/priority)")
    col1, col2, col3, col4, col5 = st.columns(5)
    with col1:
        st.markdown('<div style="background:#dc3545;padding:10px;border-radius:6px;color:white;text-align:center;">Overdue</div>', unsafe_allow_html=True)
        st.caption("Red = past due")
    with col2:
        st.markdown('<div style="background:#fd7e14;padding:10px;border-radius:6px;color:white;text-align:center;">Due Soon</div>', unsafe_allow_html=True)
        st.caption("Orange = due in next few days")
    with col3:
        st.markdown('<div style="background:#198754;padding:10px;border-radius:6px;color:white;text-align:center;">Completed</div>', unsafe_allow_html=True)
        st.caption("Green = completed or resolved")
    with col4:
        st.markdown('<div style="background:#0d6efd;padding:10px;border-radius:6px;color:white;text-align:center;">Assigned</div>', unsafe_allow_html=True)
        st.caption("Blue = assigned to you/team")
    with col5:
        st.markdown('<div style="background:#6c757d;padding:10px;border-radius:6px;color:white;text-align:center;">On Hold</div>', unsafe_allow_html=True)
        st.caption("Gray = waiting or inactive")

    st.subheader("My Patients")
    st.markdown("Manage your assigned patients. Sort by Priority or Next Task, open patient detail to document notes and advance the workflow stage.")
    import pandas as pd
    sample_patients = pd.DataFrame([
        {"Patient":"Jane Doe","Priority":"High","Status":"Overdue","Workflow Stage":"Follow-up","Next Task":"Phone Review","Due Date":"2025-11-20"},
        {"Patient":"Mark Lee","Priority":"Medium","Status":"Due Soon","Workflow Stage":"Assessment","Next Task":"Lab Review","Due Date":"2025-11-22"},
        {"Patient":"Ava Chen","Priority":"Low","Status":"Assigned","Workflow Stage":"Maintenance","Next Task":"Chart Update","Due Date":"2025-11-28"},
    ])
    st.dataframe(sample_patients, use_container_width=True)

    st.subheader("Phone Reviews")
    st.markdown("Work the call queue. Filter by due date (e.g., Today), click a call to document outcome, schedule the next follow-up.")

    st.subheader("Task Review")
    st.markdown("Review tasks assigned to you. Filter by status and type; complete or reassign as needed.")

    st.info("If any color/label differs from your actual app semantics, tell me the exact mapping and I’ll align the legend and highlights immediately.")


def render_coordinator_help_elements():
    import streamlit as st
    st.header("Coordinator Help")
    st.markdown("Two main work areas: My Patients and Phone Reviews. Team Management is available for LC/CM roles.")

    st.subheader("Color legend (status/priority)")
    col1, col2, col3, col4, col5 = st.columns(5)
    with col1:
        st.markdown('<div style="background:#dc3545;padding:10px;border-radius:6px;color:white;text-align:center;">Escalation/Overdue</div>', unsafe_allow_html=True)
    ...
    st.subheader("My Patients")
    st.markdown("Triage and coordinate follow-ups. Sort by priority, open patient details, record outcomes, and update next task.")

    st.subheader("Phone Reviews")
    st.markdown("Batch process call reviews due today or high priority. Document outcomes and schedule next steps.")

    st.subheader("Team Management (LC/CM)")
    st.markdown("Balance workloads, reassign patients, and review coordinator/task metrics.")

    st.info("Provide your exact color semantics or edge cases (e.g., \'attention\' vs \'overdue\') and I’ll update the legend and badges accordingly.")
