...
def create_coordinator_table(conn, year, month):
    ...
    """)
    conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{table_name}_coordinator ON {table_name}(coordinator_id)")
    return table_name
...
def build_provider_map(conn):
    ...
    cursor = conn.execute("SELECT user_id, first_name, last_name, full_name, username, alias FROM users WHERE status != 'deleted'")
    ...
    for row in users:
        ...
        if ln:
            code = ln[:3].upper()
            if code not in provis: provis[code] = uid
        # Username parts
        parts = uname.split('.')
        for p in parts:
            if len(p) >= 3: provis[p[:3].upper()] = uid

        # Also map full username and alias for exact matches (like AteDi000)
        if uname: provis[uname.upper()] = uid
        if alias: provis[alias.upper()] = uid
        ...
        if fn and ln:
            ...
            provis[normalize_name_key(l_f)] = uid
                ...
                cursor = conn.execute("SELECT staff_code, user_id, mapping_type FROM staff_code_mapping WHERE mapping_type IN ('COORDINATOR', 'PROVIDER')")
                ...
                for staff_code, user_id, mapping_type in staff_mappings:
                    ...
                    if user_id not in id_to_name:
                        ...
                        user_cursor = conn.execute("SELECT full_name, first_name, last_name FROM users WHERE user_id = ?", (user_id,)).fetchone()

                        if user_cursor:

                            full_name_from_users = user_cursor[0] or f"{user_cursor[1]} {user_cursor[2]}".strip()
...
def create_patient_assignments_table(conn):
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_patient_assignments_patient ON patient_assignments(patient_id)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_patient_assignments_provider ON patient_assignments(provider_id)")

def build_first_name_provider_map(conn):
    """Build mapping from provider first names to user_id"""
    first_name_map = {}
    cursor = conn.execute("SELECT user_id, first_name, full_name FROM users WHERE status != 'deleted'")
    users = cursor.fetchall()
...
def process_provider_assignments(csv_dir, conn):
    ...
    for file_path in assignment_files:
        ...
        provider_first_name = basename.replace('.csv', '').strip().replace(' ', '').upper()
        ...
        try:
            df = pd.read_csv(file_path, encoding='utf-8', on_bad_lines='skip', header=None)
            if df.empty:
                continue
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    patient_id = f"{last_name.upper()} {first_name.upper()} {dob}".strip()
                    if len(patient_id) < 5:
                        continue
...
def process_psl(file_path, conn, provider_map, id_to_name):
    ...
    try:
        ...
        if match: prov_code_file = match.group(1)
        ...
        for _, row in df.iterrows():
            ...
            if table not in records_by_table: records_by_table[table] = []

            # Provider
            p_code_raw = str(row.get('Prov','')).strip().upper()
            p_code = p_code_raw.replace('ZEN-','').strip()
            if not p_code and prov_code_file: p_code = prov_code_file
            ...
            provider_name = id_to_name.get(p_id, None) if p_id else None

            records_by_table[table].append((
                p_id, provider_name, pat_name, pat_name, dos.strftime('%Y-%m-%d'),
                str(row.get('Service','')), str(row.get('Notes','')),
                row.get('Minutes', 0), str(row.get('Coding',''))
            ))
        ...
        for table, recs in records_by_table.items():
            ...
            inserted = conn.executemany(f"INSERT OR IGNORE INTO {table} (provider_id, provider_name, patient_id, patient_name, task_date, task_description, notes, minutes_of_service, billing_code) VALUES (?,?,?,?,?,?,?,?,?)", recs).rowcount
            count += inserted
...
def process_rvz(file_path, conn, provider_map, id_to_name):
    ...
    try:
        ...
        if match_cm:
            ...
            print(f"  CMLog file detected - using coordinator from filename: {coordinator_from_filename}")
        else:
        ...
        for _, row in df.iterrows():
            ...
            if not prov_val: prov_val = str(row.get('Staff', '')).strip() # CMLog uses Staff

            if not pt_name or not prov_val or "Place holder" in str(pt_name) or "Last, First DOB" in str(pt_name):
                continue
            ...
            coordinator_name = id_to_name.get(p_id, None) if p_id else None

            records_by_table[table].append((
                p_id, coordinator_name, pt_name, dos.strftime('%Y-%m-%d'), mins,
                str(row.get(col_type,'')), str(row.get(col_notes,''))
            ))
        ...
        for table, recs in records_by_table.items():
            ...
            # to prevent duplicates while preserving existing data
            inserted = conn.executemany(f"INSERT OR IGNORE INTO {table} (coordinator_id, coordinator_name, patient_id, task_date, duration_minutes, task_type, notes) VALUES (?,?,?,?,?,?,?)", recs).rowcount
            count += inserted
            if inserted < len(recs):
                print(f"    Skipped {len(recs) - inserted} duplicate records")
        return count
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if assigned_prov:
                ...
                if not provider_id:
                    provider_id = provider_map.get(normalize_name_key(assigned_prov)) # Try normalized
                if not provider_id and len(assigned_prov) >= 3:
                     provider_id = provider_map.get(assigned_prov[:3]) # Try 3-char fallback
            ...
            patients_data.append((
                ...
                str(row.get('Initial TV\nNotes', '')).strip() if pd.notna(row.get('Initial TV\nNotes')) else None,
                str(row.get('Initial TV\nProv', '')).strip() if pd.notna(row.get('Initial TV\nProv')) else None
            ))
        ...
        if duplicate_count > 0:
            print(f"  ⚠️  Found {duplicate_count} duplicate patient_ids (appended -1, -2, etc.)")
...
def main():
    ...
    for f in psl_files: total_psl += process_psl(f, conn, pmap, id_to_name)
    for f in rvz_files: total_rvz += process_rvz(f, conn, pmap, id_to_name)
    for f in cmlog_files: total_rvz += process_rvz(f, conn, pmap, id_to_name) # Process CMLog same as RVZ
    ...
    print(f"  Total Records:        {total_assignments + total_patients + total_psl + total_rvz}")
    print("="*60 + "\n")
