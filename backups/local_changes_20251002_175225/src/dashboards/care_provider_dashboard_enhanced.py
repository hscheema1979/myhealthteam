...
def show_provider_onboarding_queue(user_id, onboarding_queue):
    ...
    for i, task_entry in enumerate(st.session_state[session_key]):
        if i < len(onboarding_queue):  # Only show tasks for current queue
            ...
            completion_key = f"completing_task_{i}_{task_entry.get('onboarding_id')}"
            
            # Check if this task is being processed
            if completion_key in st.session_state and st.session_state[completion_key]:
                # Task is being processed, show processing state
                st.info(f"Processing completion for {task_entry['patient_name']}...")
                
                # Complete the task processing
                try:
                    # Get provider_id
                    ...
                    if provider_id:
                        ...
                        if success:
                            ...
                            # Clear the completion flag and refresh
                            st.session_state[completion_key] = False
                            # Remove this patient from session state to prevent showing again
                            if session_key in st.session_state:
                                del st.session_state[session_key]
                            st.rerun()
                        else:
                ...
                if st.button(f"Complete Initial TV Visit {i+1}", key=f"complete_onb_task_{i}", type="primary"):
                    if task_entry.get('patient_name') and task_entry.get('task_type'):
                        # Set the completion flag to trigger processing on next run
