...
def test_with_small_data():
    ...
    if (test_dir / "ZMO_Main_test.csv").exists():
        ...
        print("   ✅ Created patients/ZMO_Main.csv")
    ...
    result = os.system("python scripts/unified_import.py --start-date 2025-11-01 --no-backup")
    ...
    try:
        ...
        success = result == 0 and coord_count > 0 and prov_count > 0
        if success:
            print(f"\n✅ SUCCESS: Unified import processed small test data!")
            return True
        else:
            print(f"\n❌ FAILURE: Import did not complete successfully")
            return False
